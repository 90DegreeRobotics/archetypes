...
- **Artifact return — Comfy-only (new 2026-07-12):** The game now requests the fast ComfyUI path only (`render-still` with `quick:false` + `comfyui_url` on `:8000` + install dir + checkpoint), never the slow Blender path. A verified returned PNG is staged into the asset tree and displayed in-game with its provenance; if the render cannot be produced the chamber fails closed with a precise reason and shows no image. (See blocker: the live Comfy render currently fails on a Chronos Director `codex.db` error.)
...
## Blockers
- **Live Comfy render blocked by Chronos.** The engine's Comfy request is accepted (a job id is returned and the chamber reaches `ArtifactPending`), but the Director's render job fails with `Error: opening codex at "data/codex.db"` — a Chronos Director runtime/working-directory issue, not the engine. The Director likely needs to be (re)started from `C:\chronos`. Until then the artifact step fails closed in-game.
