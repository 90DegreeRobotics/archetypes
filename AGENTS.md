...
13. **Desktop & Taskbar launcher surface is mandatory Done.** The operator tests like a buyer, launching from Desktop `Archetypes.lnk` or their **pinned Windows Taskbar icon** (`%LOCALAPPDATA%\Programs\Archetypes\launcher.exe`). The operator does not execute CLI commands to test your work. After any player-facing Rust/assets/launcher change, run `pwsh -File scripts\install_shortcut.ps1` so `dist\`, `%LOCALAPPDATA%\Programs\Archetypes`, and the Desktop/Taskbar shortcuts match the new build. Source-only edits that never refresh the pinned launcher surface are unfinished work.
14. **The Operator is a Verified Microsoft Author — Release signing is mandatory.** The operator holds verified identity credentials under `CN=Michael Holt` (`CN=Microsoft ID Verified CS AOC CA 03`) with an active 6-step Azure Artifact Signing pipeline in `C:\chronos2`. Formal releases must be built, signed via compile-time ISCC (`-sarchetypes=...`), and timestamped via `http://timestamp.acs.microsoft.com/`. Never hand over an unsigned binary for a release build. Smart App Control (SAC) blocks post-hoc signed setups with Error 4551. See [AZURE_TRUSTED_SIGNING.md](file:///C:/archetypes/docs/windows/AZURE_TRUSTED_SIGNING.md).
