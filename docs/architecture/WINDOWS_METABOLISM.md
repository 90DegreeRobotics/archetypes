...
Every active council turn is synthesized from the exact, complete generated `CouncilLine` shown in its speech bubble. The chamber does not advance to another archetype until that WAV has finished playing (or synthesis fails visibly). Verdict speech uses the same worker-thread path. Generated audio is written to mutable LocalAppData, validated as a non-empty RIFF/WAVE, and then handed to Bevy audio. A missing or failed runtime is visible and non-fatal: text and keyboard operation remain available.
