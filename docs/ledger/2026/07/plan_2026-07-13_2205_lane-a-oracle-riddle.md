...
- [ ] Action: Make `Oracle Riddle` playable in `ModeRegistration` and route it from `boot.rs`.
...
- [ ] Action: Create `oracle_riddle` module and plugin.
...
- [ ] Action: Add Oracle mode specific States and Systems.
...
- [ ] Action: Compute cosine similarity between guess words and target words using `services::llm::embed`. Add pure deterministic tests for prompt selection shape, scoring partial/exact matches, cosine helper, mode registry, and ledger payload shape.
...
- Tests added for Oracle Riddle's exact/partial scoring, cosine math, registry availability, and prompt shape.
